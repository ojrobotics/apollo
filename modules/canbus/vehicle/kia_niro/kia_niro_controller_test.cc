...
class Kia_niroControllerTest : public ::testing::Test {
 public:
  virtual void SetUp() {
...
TEST_F(Kia_niroControllerTest, Status) {
  controller_.Init(params_, &sender_, &msg_manager_);
...
TEST_F(Kia_niroControllerTest, UpdateDrivingMode) {
  controller_.Init(params_, &sender_, &msg_manager_);
  controller_.set_driving_mode(Chassis::COMPLETE_AUTO_DRIVE);
