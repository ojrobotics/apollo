...
ErrorCode Kia_niroController::Init(
    const VehicleParameter &params,
    CanSender<::apollo::canbus::ChassisDetail> *const can_sender,
    MessageManager<::apollo::canbus::ChassisDetail> *const message_manager) {
  if (is_initialized_) {
    AINFO << "kia_niroController has already been initiated.";
  ...
  if (brake_60_ == nullptr) {
    AERROR << "Brake60 does not exist in the kia_niroMessageManager!";
    return ErrorCode::CANBUS_ERROR;
  }
  ...
  if (throttle_62_ == nullptr) {
    AERROR << "Throttle62 does not exist in the kia_niroMessageManager!";
    return ErrorCode::CANBUS_ERROR;
  }
  ...
  if (steering_64_ == nullptr) {
    AERROR << "Steering64 does not exist in the kia_niroMessageManager!";
    return ErrorCode::CANBUS_ERROR;
  }
  ...
  // if (gear_66_ == nullptr) {
  //   AERROR << "Gear66 does not exist in the kia_niroMessageManager!";
  //   return ErrorCode::CANBUS_ERROR;
  // }
  ...
  //   AERROR << "Turnsignal68 does not exist in the kia_niroMessageManager!";
  //   return ErrorCode::CANBUS_ERROR;
  // }
 ...
 // can_sender_->AddMessage(Gear66::ID, gear_66_, false);             //Omar check this
 // can_sender_->AddMessage(Turnsignal68::ID, turnsignal_68_, false);  //Omar check this
...
bool Kia_niroController::Start() {
  if (!is_initialized_) {
    AERROR << "kia_niroController has NOT been initiated.";
    return false;
  }
...
void Kia_niroController::Stop() {
  if (!is_initialized_) {
    AERROR << "kia_niroController stops or starts improperly!";
    return;
  }
  ...
  if (thread_ != nullptr && thread_->joinable()) {
    thread_->join();
    thread_.reset();
    AINFO << "kia_niroController stopped.";
  }
}

Chassis Kia_niroController::chassis() {
  chassis_.Clear();
  ...
  chassis_.set_driving_mode(driving_mode());                       //Omar: this is to set to driving_mode
  ...
  // }
  // 5
  if (chassis_detail.has_vehicle_spd() &&                                 // Omar check this
  ...
  // 7
  // kia_niro only has fuel percentage
  // to avoid confusing, just don't set
  chassis_.set_fuel_range_m(0);
  // 8

    if (chassis_detail.has_gas() && chassis_detail.gas().has_throttle_output()) {     // set throttle
  ...
  // 9
  if (chassis_detail.has_brake() && chassis_detail.brake().has_brake_output()) {             // set brake
 ...
 // if (chassis_detail.has_eps() && chassis_detail.eps().has_steering_angle()) {
    if (chassis_detail.eps().has_steering_angle()) {                                          // set steering angle // Omar check
  ...
  //     chassis_detail.light().has_kia_niro_lamp_type()) {
  //   chassis_.mutable_signal()->set_high_beam(
  //       chassis_detail.light().kia_niro_lamp_type() == Light::BEAM_HIGH);
  // } else {
  ...
  // 19, kia_niro wiper is too complicated
  // 24
  if (chassis_detail.has_eps() && chassis_detail.eps().has_timestamp_65()) {              // set steering timestamp
  ...
  if (chassis_detail.has_surround()) {                                               // Surround information
...
void Kia_niroController::Emergency() {
...
ErrorCode Kia_niroController::EnableAutoMode() {
  if (driving_mode() == Chassis::COMPLETE_AUTO_DRIVE) {
    AINFO << "Already in COMPLETE_AUTO_DRIVE mode";
    return ErrorCode::OK;
  }
  brake_60_->set_enable();                                  //enable brake, throttle and steering
  ...
  set_driving_mode(Chassis::COMPLETE_AUTO_DRIVE);                          // enable complete_auto_drive
  AINFO << "Switch to COMPLETE_AUTO_DRIVE mode ok.";
  return ErrorCode::OK;
}

ErrorCode Kia_niroController::DisableAutoMode() {
  ResetProtocol();
  can_sender_->Update();
...
ErrorCode Kia_niroController::EnableSteeringOnlyMode() {                        // enable Steering Only Mode
  if (driving_mode() == Chassis::COMPLETE_AUTO_DRIVE ||
  ...
  brake_60_->set_disable();                                                  //disable brake, throttle and steering
...
ErrorCode Kia_niroController::EnableSpeedOnlyMode() {                             // enable Speed Only Mode
  if (driving_mode() == Chassis::COMPLETE_AUTO_DRIVE ||
  ...
  set_driving_mode(Chassis::AUTO_SPEED_ONLY);                                       // enable Speed Only Mode
  AINFO << "Switch to AUTO_SPEED_ONLY mode ok.";
  return ErrorCode::OK;
}

// NEUTRAL, REVERSE, DRIVE
void Kia_niroController::Gear(Chassis::GearPosition ref_gear_position) {
  if (driving_mode() != Chassis::COMPLETE_AUTO_DRIVE &&
      driving_mode() != Chassis::AUTO_SPEED_ONLY) {
...
// -> pedal
void Kia_niroController::Brake(double pedal) {
  if (driving_mode() != Chassis::COMPLETE_AUTO_DRIVE &&
      driving_mode() != Chassis::AUTO_SPEED_ONLY) {
    AINFO << "The current drive mode does not need to set acceleration.";
    return;
  }
  brake_60_->set_pedal(pedal);                                                  //set brake padel
}

// drive with old acceleration
// gas:0.00~99.99 unit:%
void Kia_niroController::Throttle(double pedal) {
  if (driving_mode() != Chassis::COMPLETE_AUTO_DRIVE &&
      driving_mode() != Chassis::AUTO_SPEED_ONLY) {
    AINFO << "The current drive mode does not need to set acceleration.";
    return;
  }
  throttle_62_->set_pedal(pedal);                                             // set pedal
}

// drive with acceleration/deceleration
// acc:-7.0 ~ 5.0, unit:m/s^2
void Kia_niroController::Acceleration(double acc) {
  if (driving_mode() != Chassis::COMPLETE_AUTO_DRIVE &&
      driving_mode() != Chassis::AUTO_SPEED_ONLY) {
    AINFO << "The current drive mode does not need to set acceleration.";
    return;
...
// kia_niro default, -470 ~ 470, left:+, right:-
// need to be compatible with control module, so reverse
// steering with old angle speed
// angle:-99.99~0.00~99.99, unit:%, left:-, right:+
void Kia_niroController::Steer(double angle) {
  if (driving_mode() != Chassis::COMPLETE_AUTO_DRIVE &&
      driving_mode() != Chassis::AUTO_STEER_ONLY) {
  ...
  // reverse sign
  steering_64_->set_steering_angle(real_angle)->set_steering_angle_speed(200);             // set steering angle/speed
}

// steering with new angle speed
// angle:-99.99~0.00~99.99, unit:%, left:-, right:+
// angle_spd:0.00~99.99, unit:deg/s
void Kia_niroController::Steer(double angle, double angle_spd) {
  if (driving_mode() != Chassis::COMPLETE_AUTO_DRIVE &&
      driving_mode() != Chassis::AUTO_STEER_ONLY) {
    AINFO << "The current driving mode does not need to set steer.";
    return;
  ...
  steering_64_->set_steering_angle(real_angle)
      ->set_steering_angle_speed(real_angle_spd);                                        // set steering angle/speed with new angle speed
}

void Kia_niroController::SetEpbBreak(const ControlCommand &command) {
...
void Kia_niroController::SetBeam(const ControlCommand &command) {
...
void Kia_niroController::SetHorn(const ControlCommand &command) {
...
void Kia_niroController::SetTurningSignal(const ControlCommand &command) {
  // Set Turn Signal
...
void Kia_niroController::ResetProtocol() {
  message_manager_->ResetSendMessages();
}

bool Kia_niroController::CheckChassisError() {
  ChassisDetail chassis_detail;
  message_manager_->GetSensorData(&chassis_detail);
...
void Kia_niroController::SecurityDogThreadFunc() {
  ...
  std::chrono::duration<double, std::micro> default_period{50000};                  // 50000
  int64_t start = cyber::Time::Now().ToMicrosecond();

  int32_t speed_ctrl_fail = 0;
  int32_t steer_ctrl_fail = 0;

  while (can_sender_->IsRunning()) {                                   // further checking
    ...
    if (elapsed < default_period) {
      ...
      AERROR_EVERY(100)
          << "Too much time consumption in Kia_niroController looping process:"
          << elapsed.count();
      start = end;
    }
  }
...
bool Kia_niroController::CheckResponse(const int32_t flags, bool need_wait) {
  // for kia_niro, CheckResponse commonly takes 300ms. We leave a 100ms buffer
  // for it.
...
void Kia_niroController::set_chassis_error_mask(const int32_t mask) {
  std::lock_guard<std::mutex> lock(chassis_mask_mutex_);
  chassis_error_mask_ = mask;
}

int32_t Kia_niroController::chassis_error_mask() {
  std::lock_guard<std::mutex> lock(chassis_mask_mutex_);
  return chassis_error_mask_;
}

Chassis::ErrorCode Kia_niroController::chassis_error_code() {
  std::lock_guard<std::mutex> lock(chassis_error_code_mutex_);
  return chassis_error_code_;
}

void Kia_niroController::set_chassis_error_code(
    const Chassis::ErrorCode &error_code) {
  std::lock_guard<std::mutex> lock(chassis_error_code_mutex_);
  chassis_error_code_ = error_code;
}

// bool Kia_niroController::CheckSafetyError(
